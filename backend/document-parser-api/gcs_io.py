...
def download_blob_to_tempfile(gcs_uri: str, temp_file):
    """Downloads a blob from GCS URI to a local temp file object."""
    logger.info(f"Downloading {gcs_uri} to temp file...")
    storage_client = storage.Client()
    try:
        ...
        logger.exception(f"Failed to download blob to temp file from {gcs_uri}")
        raise